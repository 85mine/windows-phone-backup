...
namespace ToolStackPNGWriterLib
{
    ...
    public static class PNGWriter
    {
        ...
        */
        ...
        /// <summary>
        ///     Detects the color order of a stored byte array.  Byte order may change between platforms, you should call this once
        ///     before writting a PNG or if you have any issues with colors changing.
        /// </summary>
        ...
        private static void WriteDataChunksUncompressed()
        {
            ...
            int blockRemainder = 0;
                // How much of the current block we have left, 0 to start so we write the block header out on the first block.
            int currentBlock = 0;
                // The current block we're working on, start with 0 as we increment in the first pass thorugh.
            int dataPointer = 2;
                // A pointer to where we are in the data array, start at 2 as we 'wrote' two bytes a few lines ago.
            int pixelSource = 0; // The current pixel we're working on from the image.
            var pixel = new byte[4]; // Temporary storage to store the current pixel in as a byte array.
